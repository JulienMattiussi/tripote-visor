...
- **State** - `src/i18n/store.js` exports module-scoped `locale` and `currency` refs (purely i18n + currency formatting; modal state lives in `src/state/modals.js`, profile-data accessors in `src/lib/profiles.js`). Both refs persist to `sessionStorage` (`tv_locale`, `tv_currency`) - they survive a refresh in the same tab but reset when the tab closes, no cross-session tracking. `setLocale`/`setCurrency` reject unknown values.
